...
var WYSIWYG = {
	...
	Settings: function() {
		...
		// Images Directory
		this.ImagesDir = "http://localhost/webcode3/js/wysiwyg/images/";
		
		// Popups Directory
		this.PopupsDir = "http://localhost/webcode3/js/wysiwyg/popups/";
		
		// CSS Directory File
		this.CSSFile = "http://localhost/webcode3/js/wysiwyg/styles/wysiwyg.css";		
	...
	_display: function(n, settings) {
		...
		var iframe = '<div cellpadding="0" cellspacing="0" border="0" style="width:' + currentWidth + '; height:' + currentHeight + ';" class="tableTextareaEditor">\n'
	...
	_generate: function(n, settings) {
	    ...
	    var editor = "";
	    editor += '<div id="wysiwyg_div_' + n + '">';
	    editor += '<div class="tableTextareaEditor" id="wysiwyg_table_' + n + '">';
	    editor += '';
		...
		for (var j = 0; j < this.config[n].Toolbar.length;j++) { 
			if(this.config[n].Toolbar[j] && this.config[n].Toolbar[j].length > 0) {
			    ...
			    editor += '<div class="toolbar1" style="width:100%;" id="toolbar' + j + '_' + n + '">';
	    		editor += '<img src="' + this.config[n].ImagesDir + 'seperator2.gif" alt="" hspace="3">';
	    ...
	    if(this.config[n].StatusBarEnabled) {
		    editor += '<div class="wysiwyg-statusbar" style="height:10px;" id="wysiwyg_statusbar_' + n + '">&nbsp;';    
		}
	    editor += '</div>';
	    editor += '</div>';
...
var WYSIWYG_Core = {
	...
	isBrowserCompatible: function() {
		...
		// ((navigator.userAgent.indexOf('Safari') != -1 ) || !document.getElementById || !document.designMode){   
...
var WYSIWYG_ContextMenu = {
	...
	output: function (n) {
		...
		this.html += '<div class="wysiwyg-context-menu" border="0" cellpadding="0" cellspacing="0">';
	...
	addItem: function(n, cmd, icon, title, disabled) {
		...
		if(disabled) {
			item += '';
			item += '<div class="icon"><a href="javascript:WYSIWYG.execCommand(\'' + n + '\',\'' + cmd + '\', null);"><img src="' + icon.enabled + '" border="0"></a></div>';
			item += '<div onmouseover="this.className=\'mouseover\'" onmouseout="this.className=\'\'" onclick="WYSIWYG.execCommand(\'' + n + '\', \'' + cmd + '\', null);WYSIWYG_ContextMenu.close();"><a href="javascript:void(0);">' + title + '</a>';
			item += '</div>';
		}
		else {
			item += '';
			item += '<div class="icon"><img src="' + icon.disabled + '" border="0"></div>';
			item += '<div onmouseover="this.className=\'mouseover\'" onmouseout="this.className=\'\'"><span class="disabled">' + title + '</span></div>';
			item += '';
		}
	...
	addSeperator: function() {
		var output = '';
		output += '';
		output += '<hr size="1" color="#C9C9C9" width="95%">';
		output += '';
		this.html += output;
	}
}
